...
from uptrain import Settings as UptrainSettings
...
class EvalAgent(RoutedAgent):
    def __init__(self) -> None:
        ...
        uptrain_settings = UptrainSettings(
            model="gpt-4", openai_api_key=settings.OPENAI_API_KEY
        )
    ...
    async def evaluate_answer(self, message: Message, ctx: MessageContext) -> Message:
        try:
            ...
            return Message(
                content=json.dumps(
                    {"score": score, "reasoning": reasoning, "error": None}
                )
            )
            ...
            return Message(
                content=json.dumps({"score": 0, "reasoning": "", "error": str(e)})
            )
