...
class Settings(BaseSettings):
    ...
    NOTION_MCP_TOKEN: Optional[str] = Field(None, description="Token for Notion MCP Gateway")
    ...
    BACKEND_URL: str = Field(default="http://127.0.0.1:8001", description="Backend service URL")
    
    # Database Settings
    DATABASE_URL: str = Field(default="sqlite:///./memory.db", description="Database connection URL")
    CHROMA_DB_PATH: str = Field(default="services/knowledge-base-chroma-index/chroma_db", 
    ...
    # WebRAG Settings
    WEBRAG_LLM_DEFAULT_MODEL: str = Field(default="llama-3.3-70b-versatile", 
                                         description="Default LLM model for WebRAG")
    WEBRAG_EMBED_DEFAULT_MODEL: str = Field(default="text-embedding-ada-002", 
    ...
    WEBRAG_MAX_SEARCH_RESULTS: int = Field(default=10, description="Maximum number of search results")
    WEBRAG_MAX_VIDEO_RESULTS: int = Field(default=5, description="Maximum number of video results")
    WEBRAG_MAX_GENERAL_RESULTS: int = Field(default=5, description="Maximum number of general results")
    WEBRAG_TOP_K: int = Field(default=3, description="Number of top results to consider")
    
    # Cache Settings
    CACHE_TTL: int = Field(default=3600, description="Cache time-to-live in seconds")
    
    # Rate Limiting
    RATE_LIMIT_REQUESTS: int = Field(default=100, description="Number of requests allowed per time window")
