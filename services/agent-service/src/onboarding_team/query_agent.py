...
class QueryAgent(RoutedAgent):
    ...
    async def handle_message(self, message: Message, ctx: MessageContext) -> Message:
        ...
        aggregated_json = await self.aggregate_results(plan["user_query"], results, plan["execution_order"]["aggregation"])
        aggregated_json = json.loads(aggregated_json)

        payload = {
        "individual_results": results,
        "aggregated_results": aggregated_json["aggregated_results"],
        "confidence_score":   aggregated_json["confidence_score"],
        }
        return Message(content=json.dumps(payload))
    
    
    async def execute_query(self, component: Dict[str, Any]) -> Any:
        sub_query = component.get("sub_query", "")
    ...
    async def aggregate_results(self, user_query: str, results: Dict[str, Any], strategy: Optional[str] = None) -> str:
        prompt = f'''
