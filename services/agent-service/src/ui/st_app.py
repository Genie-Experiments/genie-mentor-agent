...
async def stream_response(query: str, session_id: str) -> None:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            ...
            editor_container, editor_content = create_agent_container("Editor Agent", "✏️")
            ...
            async for chunk in response.content:
                try:
                    ...
                    if 'planner_output' in data:
                        ...
                        # Update planner output - show original plan in bullet points
                        planner_content.markdown(f"""

                        **User Query:
                        - {plan_dict.get('user_query', '')}

                        **Data Sources:**
                        {chr(10).join(f"- {source}" for source in plan_dict.get('data_sources', []))}

                        **Query Components:**
                        {chr(10).join(f"- {component.get('sub_query', '')}" for component in plan_dict.get('query_components', []))}
                        """)
                        
                        # Update refiner output - show refined plan and metadata in bullet points
                        if 'refiner_metadata' in plan_dict:
                            refiner_data = plan_dict['refiner_metadata']
                            # Ensure refined_plan is parsed correctly if it's a JSON string
                            try:
                                refined_plan_dict = json.loads(refiner_data.get('refined_plan', '{}'))
                            ...
                            **User Query:**
                            - {refined_plan_dict.get('user_query', '')}

                            **Data Sources:**
                            {chr(10).join(f"- {source}" for source in refined_plan_dict.get('data_sources', []))}

                            **Query Components:**
                            {chr(10).join(f"- {component.get('sub_query', '')}" for component in refined_plan_dict.get('query_components', []))}
                        ...
                        # Update editor output - show final answer
                        if 'evaluation_results' in plan_dict:
                            eval_data = plan_dict['evaluation_results']
                            editor_content.markdown(f"""
                            
                            **Final Answer:**
                            {eval_data.get('final_answer', '')}
                            """)

                        # Update evaluation output - show evaluation data (excluding final_answer)
                        if 'evaluation_results' in plan_dict:
                            eval_data = plan_dict['evaluation_results']
