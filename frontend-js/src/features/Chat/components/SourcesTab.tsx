...
import type { ExecutorAgent, KnowledgebaseMetadata, GitHubMetadata, NotionMetadata, WebsearchMetadata } from '../../../lib/api-service';
...
const SourcesTab: React.FC<SourcesTabProps> = ({ executorAgent }) => {
  ...
  const renderKnowledgeBaseSources = (metadata: KnowledgebaseMetadata[]) => (
    <ol style={{ paddingLeft: 0 }}>
      {metadata.map((item, index) => (
        <li key={index} style={{ marginBottom: '18px', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', listStyle: 'none' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px' }}>
            <span style={{ ...itemDetailStyle, fontWeight: 500, fontSize: '16px', width: '24px', flexShrink: 0 }}>{index + 1}.</span>
        ...
        <li key={index} style={{ marginBottom: '18px', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', listStyle: 'none' }}>
          ...
          <div style={{ marginLeft: 32, width: '100%' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <a href={item.url} target="_blank" rel="noopener noreferrer" style={linkStyle}>{item.url}</a>
            </div>
  ...
  const renderGitHubSources = (metadata: GitHubMetadata[]) => renderWebSources(mapGitHubToWebsearchMetadata(metadata));

  const renderNotionSources = (metadata: NotionMetadata[]) => renderWebSources(mapNotionToWebsearchMetadata(metadata));

  return (
    <div className="flex flex-col gap-4 w-full text-[#002835] font-['Inter']">
